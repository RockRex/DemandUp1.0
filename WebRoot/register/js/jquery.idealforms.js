...
$.idealforms.errors = {
  ...
  username: '必须由6到20位字母和数字组成，并以字母开头。',
  pass: '必须至少6个字符长，并且至少包含一个数字，一个大写字母和一个小写字母.',
  realName: '此处是必填的,且必须为真实姓名.',
  strongpass: '必须至少为8个字符长，至少包含一个大写字母和一个小写字母和一个数字或特殊字符.',
  email: '必须是一个有效的e -mail地址. <em>(e.g. user@gmail.com)</em>',
  phone: '必须是一个有效的手机. <em>(如. 13012349876)</em>',
  zip: '必须是一个有效的中国邮编.<em>(如. 076543)</em>',
  url: '必须是一个正确的网址. <em>(如： www.google.com)</em>',
...
var getFilters = function() {
  ...
  var filters = {
    ...
    realName: {
        regex: /.+/,
        error: $.idealforms.errors.realName
      },
    number: {
